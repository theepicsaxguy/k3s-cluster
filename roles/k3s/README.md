...
The role performs various tasks related to `k3s` [cluster](https://github.com/k3s-io/k3s/releases/tag/v1.31.4+k3s1) deployment, reset and validation. Review the [documentation](https://theepicsaxguy.com/k3s-cluster/wiki/guide/configuration/roles/k3s), for additional details.
...
| k3s_vars.cluster.tls_san | list | `["10.25.150.10"]` | Related to `server.api.host` key |
...
| k3s_vars.server.api.host | string | `"10.25.150.10"` | Related to `cluster.tls_san` key, used as frontend bind IP for Haproxy loadbalancer and populated into `/.kube/config` file |
